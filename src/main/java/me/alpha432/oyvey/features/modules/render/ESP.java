...
public class ESP extends Module
{
    ...
    public ESP() {
        ...
        this.burrow = (Setting<Boolean>)this.register(new Setting("Burrow", true));
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        if (this.burrow.getValue()) {
            this.posList.clear();
            for (final EntityPlayer player : ESP.mc.world.playerEntities) {
                if (EntityUtil.isBurrow((Entity)player)) {
                    final BlockPos blockPos = new BlockPos(player.getPositionVector());
                    this.posList.add(blockPos);
                }
            }
            for (final BlockPos blockPos2 : this.posList) {
                this.drawBox(blockPos2, this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue());
            }
        }
    }
    ...
    private void drawBox(final BlockPos blockPos, final int r, final int g, final int b, final int a) {
        ...
        final Color colorBound = new Color(r, g, b, 175);
