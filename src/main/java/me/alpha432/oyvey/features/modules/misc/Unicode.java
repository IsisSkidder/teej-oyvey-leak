...
public class Unicode extends Module {
    private static Unicode INSTANCE = new Unicode();
    public Setting<Boolean> ignore = register(new Setting("Ignore", true));
    public Setting<Boolean> lag = register(new Setting("Test", true));
    public Setting<Integer> pop_delay = register(new Setting("Delay", 2500, 0, 10000));
    ...
    public void onTotemPop(EntityPlayer player) {
        if (player.entityId != this.mc.player.entityId && !OyVey.friendManager.isFriend(player) && !this.sent.contains(player.getName()) && this.timer.passedMs(this.pop_delay.getValue()) && this.lag.getValue() && this.isOn()) {
            this.mc.player.sendChatMessage("/msg " + player.getName() + " " + this.getUnicodeMessage());
